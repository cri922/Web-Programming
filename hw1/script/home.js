...
function manageList(id,method){
  if(method=="add"){
    likesList.push({animeID : id}); 
  }else{
    let index;
    for (let i = 0; i < likesList.length; i++) {
      if(likesList[i].animeID==id){
        index=i;
        break;
      }
    }
...
function onLikeClickListener(event){
  ...
  if(img.dataset.like==="true"){
    fetch("/hw1/ajax/ajax_remove_like.php?anime="+animeID).then(onResponse).then(onRemoveJson);
  }else{
    fetch("/hw1/ajax/ajax_add_like.php?anime="+animeID).then(onResponse).then(onAddJson);
  }
}
...
function onSearchJson(json){
  ...
  for(let i=0;i<num;i++){
    ...
    link.href = "anime.php?anime="+json.data[i].mal_id;
...
function onSearchClickListener(event){
  ...
  if(animeInput.value.trim()!==""){
    ...
    fetch("/hw1/ajax/ajax_search_anime.php?q="+name_encoded+"&type=tv").then(onResponse).then(onSearchJson);
  }else{
    animeContainer.innerHTML="";
  }
}
...
function onFavouriteClickListener(event){
  ...
  fetch("/hw1/ajax/ajax_remove_like.php?anime="+animeID).then(onResponse).then(onRemoveFavouriteJson);
}
...
function onFavouriteJson(json){
  ...
  p.textContent = synopsis;
  const link = document.createElement("a");
  link.href = "anime.php?anime="+json.data.mal_id;
  link.textContent="Read more!";
  p.appendChild(link);
  div.appendChild(p);
  favourites.appendChild(div);
...
function onLikesJson(json){
  ...
  for (const ani of likesList) {
    setTimeout(function(){
      fetch("/hw1/ajax/ajax_get_anime_info.php?anime="+ani.animeID).then(onResponse).then(onFavouriteJson)
...
fetch("/hw1/ajax/ajax_get_likes.php").then(onResponse).then(onLikesJson);
